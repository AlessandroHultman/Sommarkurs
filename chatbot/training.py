...
intents: Dict[str, List[Dict[str, str]]] = json.loads(
    open('intents.json').read())
...
words_lemmatized = [lemmatizer.lemmatize(
    word) for word in words if word not in ignore_chars]
words_sorted: List[Any] = sorted(set(words_lemmatized))
classes_sorted: List[Any] = sorted(set(words_lemmatized))  # todo: fix this shit
